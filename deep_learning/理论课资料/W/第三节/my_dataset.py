...
class MNIST_Data(Dataset):
    def __init__(self, data_root):
        self.data_root = data_root  # 数据存放位置
        self.dataset = []
        class_path = os.listdir(self.data_root)  # 获取各个类别子目录
        for c in class_path:
            img_names = os.listdir(os.path.join(data_root, c))  # 获取当前类别目录下所有图像名称
            for img_name in img_names:
                path = os.path.join(data_root, c, img_name)  # 当前图像路径
                label = int(c)  # 类别标签
                self.dataset.append((path, label))  # 将路径和类别标签添加到列表中

    def __getitem__(self, idx):
        path, label = self.dataset[idx]  # 读取路径和类别标签
        img = cv2.imread(path, 0)  # 读取图像
        img = img.reshape(-1)  # 展平成向量
        img = np.float32(img / 255)  # 归一化，数值调整到0~1
...
if __name__ == '__main__':
    ...
    # 定义优化器
    opt = torch.optim.SGD(net.parameters(), lr=0.01, momentum=0.9)  # learning rate  学习率
