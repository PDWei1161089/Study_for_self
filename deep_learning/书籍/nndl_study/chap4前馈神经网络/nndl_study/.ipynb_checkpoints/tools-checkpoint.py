...
def plot(runner, fig_name):
    ...
    plt.subplot(1, 2, 2)
    # 绘制训练准确率变化曲线
    plt.plot(epochs, runner.trrain_scores, color="#8E004D", label="Train accuracy")
    # 绘制评价准确率变化曲线
    plt.plot(epochs, runner.dev_scores, color="#E20079", linestyle="--", label="Dev accurecy")
    # 绘制坐标轴和图例
    plt.ylabel("score")
    plt.xlabel("epochs")
...
def plot_training_loss_acc(runner, fig_name, 
    ...
    plt.plot(train_steps, train_losses, color=train_color, linestyle=train_linestyle, label="Trian loss")
    ...
    plt.xlabel("stpe", fontsize=fontsize)
