...
from sklearn.datasets import load_digits  # MNIST手写体数字的数据
...
def make_moons(n_samples=1000, shuffle=True, noise=None):
    ...
    # 采集第1类数据, 特征为(x, y)
    # 使用"paddle.linspace" 在0到 pi 上均匀取n_samples_out个值
    ...
    # 使用"paddle.stack"将两类特征沿维度1堆叠在一起
    X = paddle.stack(
    ...
    if noise is not None:
        # 使用"paddle.normal"生成符合正态分布的随机Tensor作为噪声, 并加给原始特征上
...
def load_data(shuffle=True):
    ...
    输出:
        - X: 特征数据, shape=[150, 4]
        - y: 标签数据, shape=[150, 3]
        - 150为数据量大小, X的shape中的4表示为鸢尾花的特征类别数, y的shape中的3表示为鸢尾花的类别数
    """
    # 加载原始数据
...
# 加载数据集
def load_data_mnist(shuffle=True):
    """
    加载MNIST手写体数字数据
    输入:
        - shuffle: 是否打乱数据, 数据类型为bool
    输出:
        - X: 特征数据, shape=[1797, 64]
        - y: 标签数据, shape=[1797, ]
    """
    # 加载原始数据
    X = np.array(load_digits().data, dtype=np.float32)
    y = np.array(load_digits().target, dtype=np.int64)
