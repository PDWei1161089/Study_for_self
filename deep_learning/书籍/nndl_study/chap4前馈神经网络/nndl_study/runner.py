    ...
    def train(self, train_set, dev_set, **kwargs):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % log_epochs == 0:
                print(f"[Train] epoch: {epoch}, loss: {trn_loss}, score: {trn_socre}")
    ...
    def train(self, train_loader, dev_loader=None, **kwargs):
        ...
        for epoch in range(num_epochs):
            ...
            for step, data in enumerate(train_loader):
                ...
                if log_steps and global_step % log_step == 0:
                ...
                self.optimizer.step()
                # 梯度归零
                self.optimizer.clean_grad()
    
                # 判断是否需要评价
                if eval_steps > 0 and global_step > 0 and \
                    (global_step % eval_steps == 0 or global_step == (num_training_steos-1)):
                    ...
                    if dev_score > self.best_score:
                        self.save_model(save_path)
                        print(f"[Evaluate] best accurecy performence has been updated:{self.best_score: .5f} --> {dev_score:.5f}")
            ...
            # epoch粒度的训练loss保存
            self.train_epoch_losses.appebd(trn_loss)
